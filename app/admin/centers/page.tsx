...
export default async function AdminCentersPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-2xl font-bold text-foreground">المراكز</h1>
        <p className="mt-1 text-sm text-muted-foreground">{centers.length.toLocaleString("ar-SA")} مركز</p>
      </div>

      <Card className="space-y-3 p-4">
        <form method="get" className="flex gap-2">
          <input type="hidden" name="status" value={status ?? ""} />
          <Input name="q" defaultValue={q ?? ""} placeholder="ابحث باسم المركز…" className="max-w-sm" />
      ...
      <Card className="overflow-hidden p-0">
        {centers.length === 0 ? (
          <EmptyState icon={Building2} title="لا توجد مراكز مطابقة" description="جرّب تعديل الفلاتر أو كلمات البحث." />
        ) : (
